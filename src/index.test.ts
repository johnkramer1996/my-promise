...
test('resolution handler is called when promise allSetled is resolved', async () => {
  return new MyPromise((res) =>
    ...
    expect(data1).toEqual({ status: State.RESOLVED, value: expectSting1 })
    expect(data2).toEqual({ status: State.RESOLVED, value: expectSting2 })
    expect(data3).toEqual({ status: State.RESOLVED, value: expectSting3 })
    expect(data4).toEqual({ status: State.RESOLVED, value: expectSting4 })
  })
})
...
test('resolution handler is called when promise allSetled is rejected', async () => {
  return new MyPromise((res) =>
    ...
    expect(data3).toEqual({ status: State.RESOLVED, value: expectSting3 })
    expect(data4).toEqual({ status: State.RESOLVED, value: expectSting4 })
    expect(data5).toEqual({ status: State.REJECTED, reason: expectSting5 })
  })
})
